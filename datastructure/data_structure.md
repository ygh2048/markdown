...
### 冒泡排序
#### 原理
重复遍历要排序的列表，依次比较相邻的两个元素，根据大小交换，两次for循环
...
### 选择排序
#### 原理
分为已排序和未排序两部分,每次从未排序中找到最小的或最大的一个元素，然后交换第一个元素，然后将其纳入已排序的末尾，重复过程直到未排序为空
#### 实现
...
- 最坏情况O(n2)
- 最好情况O(nlogn)
- 平均情况O(nlogn)  -- 实际效率可能更优与其他同时间复杂度算法
...
### 归并排序
#### 原理
同样基于分治法,将数组分解为更小的部分，排序后再合并成一个有序整体
不过是严格二分
#### 实现
```cpp
