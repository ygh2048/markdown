...
TCP协议能够保护数据不会遗失，每次发送数据都会要求确认       


TCP没有长度限制，理论可以无限长，但是为了效率一般不会超过IP数据报的长度     
...
总长度不超过65535个字节，刚好放入一个IP数据报       




# 网络层/网际层 IP层
负责在源和终点之间建立连接。包括网络寻径，流量控制，错误检查等等        
...
|标头|数据|
|-|-|
|版本 长度 IP地址|IP数据报的具体内容|
|标头长度20 - 69字节||

数据报总长度最大为65535字节         
所以如果IP数据报的长度超过了1500字节，它就需要分割成几个以太网数据帧分开发送        
