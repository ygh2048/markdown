...
## 浅拷贝和深拷贝对比
### 浅拷贝
是一种对象复制方式，其中只复制对象的成员变量值，而不是复制他们所指向的资源		

### 深拷贝
是一种对象复制方式，其中对象的成员变量值被赋值，同时资源也被复制，每个对象都有自己对立的资源副本。包含指针和动态分配内存的对象		
...
const 传指针， 表示函数不会修改指针所指向的数据
const 传引用， 表示函数不会修改原始数据，同时避开了传值的拷贝开销		
...
| 特性 | 传值 | 传引用 | 传指针 |
|------|------|--------|--------|
| **语法** | `void func(vector<int> v)` | `void func(vector<int>& v)` | `void func(vector<int>* v)` |
| **是否创建副本** | 是 | 否 | 否 |
| **修改原对象** | 不能 | 能 | 能 |
| **空值安全性** | 总是安全 | 总是安全 | 需要检查空指针 |
| **性能** | 低（需要拷贝） | 高（无拷贝） | 高（无拷贝） |
| **调用语法** | `func(vec)` | `func(vec)` | `func(&vec)` |

### 总结	
1. **传值**：适用于需要操作副本而不影响原对象的小型数据
2. **传引用**：适用于需要修改原对象或避免拷贝的大型对象（如vector）
3. **传常量引用**：适用于只需要读取数据而不需要修改的情况
4. **传指针**：适用于可能需要传递空值的情况，或者与C代码交互
