...
是MMU(操作系统内存管理子系统)的核心数据结构，实现了虚拟内存的关键功能，将进程使用的虚拟地址 转换/映射到计算机物理内存中的物理地址
...
TLB是页表专用缓存，缓存了最近使用的虚拟页到物理页帧的映射 （PTE内容）       TLB命中则直接获得物理页帧号 （PFN）获得物理地址 
CPU拿着这个物理地址去查询CPU缓存，缓存中存储着物理地址对应的数据内容        
...
1. 虚拟内存实现
2. 内存隔离和保护
   1. 页表项中包含限位（读写执行），内核可以设置这些位来防止用户程序修改内核数据或执行非代码区的数据，提高系统安全性
3. 按需分页
4. 物理内存抽象与管理
...
页表项：包含：（最底层条目，一个64位值）  
- 物理页帧号：指定该虚拟页映射到的物理内存
- 存在位：1 表示该页存在于物理内存中，0表示不在（缺页异常）
- 读写位：1 可写 0 只读
- 用户/管理位： 0 只有内核态可以访问 1 表示用户态页可以访问
- 执行禁止位：1 表示该内容不能作为代码执行
- 已访问位：1表示该页上次被清零后被读写或写入过
- 脏位：1表示该页自加载后被写入过
- 其他位
